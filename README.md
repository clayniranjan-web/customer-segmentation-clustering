...
## UMAP
Applied as a second, non-linear dimensionality reduction technique to check whether PCA's smooth, unseparated cloud was hiding real structure or accurately reflecting the data. UMAP's projection looked visually striking — several distinct blobs connected by thin bridges — a sharp contrast to PCA's continuous fan shape.

However, overlaying the K-Means, Hierarchical, and DBSCAN cluster labels onto this UMAP layout showed none of the three algorithms' cluster boundaries align with UMAP's visual blobs. This is a key finding: UMAP's apparent separation is a projection artifact (driven by its local-neighborhood-preserving nature and the min_dist parameter pulling similar points into tight visual clumps), not evidence of genuine behavioral segments. All three clustering algorithms, working directly on the actual RFM feature space, cut across UMAP's blobs rather than respecting them — reinforcing that customer behavior in this dataset is fundamentally continuous, not naturally categorical.
